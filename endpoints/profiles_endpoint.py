...
class ProfileEndpoint(AuthEndpoint):
    ...
    def get_profile_for_id(self, id_for_get):
        token = self.get_auth_token(test_data.login, test_data.password)
    ...
    def set_active_profile(self, id_for_set):
        token = self.get_auth_token(test_data.login, test_data.password)
        url = f'{test_data.url}/profiles/active'
    ...
    def deactivate_active_profile(self, id_for_deactivate):
        token = self.get_auth_token(test_data.login, test_data.password)
        url = f'{test_data.url}/profiles/active'
        headers = {'Authorization': f'Bearer {token}'}
        response = requests.delete(url, headers=headers)
